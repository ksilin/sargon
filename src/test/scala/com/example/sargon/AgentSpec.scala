...
//  http://doc.akka.io/docs/akka/current/scala/agents.html
  ...
  // can be used eg as concurrent cache: http://stackoverflow.com/questions/35006070/client-session-storage-in-an-akka-http-server
  ...
  "what is an agent and what is it for" - {
    ...
    "agents are monadic" in {
      ...
      val agent4: Agent[Int] = agent3 map (_ + 1)
      Await.result(agent4.future(), 1 second) should be(5)
      ...
      Await.result(agent5.future(), 1 second) should be(8) // 5 + 3
    }
  }
}
